...
//!	buffer with data size prototype
	class dsb
	{
	 public:
	 	dsb(unsigned s = 0);
	//! data / empty.
        inline bool empty() const { return ds == 0; }
        inline bool data () const { return ds >  0; }
    //! size of contents
        inline unsigned  size() const { return ds; }
...
//! Const char smart pointer.
	...
	//!	for explicit cast.
		inline t_ch str() const { return cc; }
	...
	//!	mutable reference access.
		inline s_cc& mr() const { return const_cast<s_cc&>(*this); }
...
//!	- does not control full state.
    ...
    //! the contents as character string.
		inline t_cc str () const { return db; }
	//!	full information.
		inline bool full() const { return ds >= mx; }
	//!	remaining space to be filled.
		inline unsigned  pot () const { return mx - ds; }
	//!	clean contents without memory shrink.
		void clean();
