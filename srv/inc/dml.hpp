...
//!	attribute stream closer.
	...
	//	output and casts
	//!	reset output buffer.
		inline virtual void clear(void) { bfout.clear(); tbf.clear(); }
	//!	retieve output buffer data.
		inline t_cc str(void) 			{ return bfout; }
	//!	retieve output buffer data size.
		inline unsigned size(void) 		{ return bfout.size(); }

		inline t_cc last(void) 			{ return tbf.last(); }
		inline bool operator==(t_cc c) 	{ return tbf.last() && tbf.last() == c; }

		inline operator bool(void) 		{ return bfout; }
		inline operator t_cc(void)		{ return bfout; }

		static dmlstream& instance(void);
//		static dmlstream& tmp_instance(void);
...
namespace tag
{
	...
	inline ttag div		(void) { return mtt(c_div); }
	inline ttag label	(void) { return mtt(c_label ); }
	inline ttag li		(void) { return mtt("li"); }
	inline ttag option	(void) { return mtt(c_option); }
	inline ttag p 		(void) { return mtt("p"); }
	inline ttag select	(void) { return mtt(c_select); }
	inline ttag span	(void) { return mtt(c_span); }
	inline ttag ul		(void) { return mtt("ul"); }

	inline stag br		(void) { return mts("br"); }
	inline stag img		(void) { return mts(c_img); }
	inline stag input	(void) { return mts(c_input); }
...
//	single (non value) attributes.
	inline saa checked (void)	{ return mas(c_checked); }
	inline saa readonly(void)	{ return mas(c_readonly); }
	inline saa selected(void)	{ return mas(c_selected); }
	inline saa multiple(void)	{ return mas(c_multiple); }
