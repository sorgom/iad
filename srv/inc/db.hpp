...
namespace db
{
	...
	//!	(precisely character after last one.)
		inline const char ldbc(void) { return cas; }
...
//!	has to be deleted after usage.
	...
	//!	number of rows retrieved.
		inline unsigned rows(void) const { return nr; }
	//!	number of columns retrieved.
		inline unsigned cols(void) const { return nc; }
	//!	column name.
		inline const s_cc& col(unsigned n) const { return bcols[n]; }
		inline const s_cc& operator[](unsigned n) const { return bcols[n]; }
	//!	next data value.
		inline const s_cc& next(void) const { return bdata[pos++]; }
	//! go back to position 0.
		inline void rewind(void) const { pos = 0; }
...
//!	sql processor class.
	...
	//!	information wether bulk updates are standing out.
		inline bool updates(void) { return bb; }
